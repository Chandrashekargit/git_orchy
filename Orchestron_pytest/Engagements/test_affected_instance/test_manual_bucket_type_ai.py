...
sca_tools = [("/home/junaid/Downloads/results_supported_by_orchy/zap.xml", "ZAP (json,xml)"),
             ...
             ("/home/junaid/Downloads/results_supported_by_orchy/OWASP Dependency Checker.xml", "OWASP Dependency"),
             ("/home/junaid/Downloads/results_supported_by_orchy/snyk.json", "snyk"),
             ("/home/junaid/Downloads/results_supported_by_orchy/WhiteSource.xml", "whitesource")
             ]
...
cwes = ["89:", "79:", "502:"]
cwes1 = ["89", "79", "502"]
scan_names = ["manual1", "manual2", "manual3"]


@mark.check_manual_ai_when_bt_is_manual
class CheckBucketTypeAffectedInstancesTests:
    def test_create_app(self, driver):
        wait = WebDriverWait(driver, 20, poll_frequency=2, ignored_exceptions=[
            NoSuchElementException, ElementNotVisibleException, ElementClickInterceptedException])

        create_apps(driver, application_name="check manual ai", url="http://demo.com")
    ...
    def test_create_manual_vul(self, driver):
        for (x, y, z) in zip(sev, cwes, scan_names):
            create_manual_vul(driver, individual_app_xpath="//label[contains(text(),'check manual ai')]",
                              Descrption="manual vul", Severity=x, cwe_num=y, scan_name=z)

    def test_create_manual_evd(self, driver):
        for cwe in cwes1:
            for manual_vul in ["//a[.="+cwe+"]/../../..//td[2]//p"]:
                open_manual_vulnerability(driver, individual_vul_xpath=manual_vul, application_name_xpath="//label[contains(text(),'check manual ai')]")
                sast_evd(driver, line_no=25, line_range="25-30", code_snippet_location="/home/junaid/Desktop/Response.txt",
                         path="Downloads/abc/def", file_name="demo", param="arg1")
                dast_evd(driver, enter_url="http://demo.com", enter_param="arg1", enter_payload="a!=1",
                         req_file_loc="/home/junaid/Desktop/request.txt", response_file_loc="/home/junaid/Desktop/Response.txt")
                sca_evd(driver, module_name="abc.jar", version_id="1.26.36", cve_id="CVE-2014-2015669")
                time.sleep(2)

    def test_upload_scans(self, driver):
        for (tool2, name2) in sca_tools:
            upload_res(driver, application="//label[contains(text(),'check manual ai')]", tool_name=name2,
                       scan_name=name2, file_loc=tool2)
    ...
    def test_create_eng(self, driver):
        ...
        create_engagement(driver, engagement_name="check manual ai", eng_descrption="check ai filters",
        which_application="check manual ai", which_scope_type="Manual")
    ...
    def test_assign_scans_and_check_ai(self, driver, engagement_name_xpath="//label[contains(text(),'check manual ai')]"):
        """
        ...
        for i in [high_severity, medium_severity, low_severity, info_severity]:
            try:
                ...
                time.sleep(2)
                stop_till_spinner_is_invisible(driver)
                total_num_of_vul = WebDriverWait(driver, 5, poll_frequency=1).until(
                    EC.presence_of_all_elements_located((By.XPATH, "//tr[@class='el-table__row']//td[2]//p")))
            except TimeoutException:
                continue
            for j in range(1, len(total_num_of_vul) + 1):
                try:
                    click_on_individual_vul = wait.until(EC.element_to_be_clickable(
                        (By.XPATH, "//tr[@class='el-table__row'][" + str(j) + "]//td[2]//p")))
                    ...
                    time.sleep(2)
                    give_total_num_of_ai = WebDriverWait(driver, 5, poll_frequency=1).until(EC.presence_of_all_elements_located((By.XPATH,
                         "//div[@class='container-fluid' and @style]//div[@class='row']//div[@class='col-sm-10']")))
                except TimeoutException:
    ...
    def test_delete_app(self, driver):
        delete_app(driver, application="//label[contains(text(),'check sca ai')]")
