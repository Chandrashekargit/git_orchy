...
sast_tools = [("/home/junaid/Downloads/results_supported_by_orchy/zap.xml", "ZAP (json,xml)"),
...
tool_names = ["Checkmarx", "Brakeman", "Bandit", "Appscan-SAST", "NodeJsScan", "Xanitizer", "HP Fortify",
              "FindSecBugs", "Veracode", "Gosec"]


@mark.check_sast_ai_when_bt_is_sast
class CheckBucketTypeAffectedInstancesTests:
    def test_create_app(self, driver):
        wait = WebDriverWait(driver, 20, poll_frequency=2, ignored_exceptions=[
            NoSuchElementException, ElementNotVisibleException, ElementClickInterceptedException])

        create_apps(driver, application_name="check sast ai", url="http://demo.com")
    ...
    def test_upload_scans(self, driver):
        for (tool2, name2) in sast_tools:
            upload_res(driver, application="//label[contains(text(),'check sast ai')]", tool_name=name2, scan_name=name2, file_loc=tool2)
    ...
    def test_create_eng(self, driver):
        ...
        create_engagement(driver, engagement_name="check sast ai", eng_descrption="check ai filters", which_application="check sast ai",
                          which_scope_type="SAST")
    ...
    def test_assign_scans_and_check_ai(self, driver, engagement_name_xpath="//label[contains(text(),'check sast ai')]"):
    ...
    def test_delete_app(self, driver):
        delete_app(driver, application="//label[contains(text(),'check sast ai')]")
