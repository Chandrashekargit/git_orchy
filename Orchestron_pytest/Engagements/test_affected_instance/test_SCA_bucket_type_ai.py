...
tool_names = ["OWASP Dependency Checker", "Snyk", "WhiteSource"]


@mark.check_sca_ai_when_bt_is_sca
class CheckBucketTypeAffectedInstancesTests:
    def test_create_app(self, driver):
        wait = WebDriverWait(driver, 20, poll_frequency=2, ignored_exceptions=[
            NoSuchElementException, ElementNotVisibleException, ElementClickInterceptedException])

        create_apps(driver, application_name="check sca ai", url="http://demo.com")
    ...
    def test_upload_scans(self, driver):
        for (tool2, name2) in sca_tools:
            upload_res(driver, application="//label[contains(text(),'check sca ai')]", tool_name=name2, scan_name=name2, file_loc=tool2)
    ...
    def test_create_eng(self, driver):
        ...
        create_engagement(driver, engagement_name="check sca ai", eng_descrption="check ai filters",
                                    which_application="check sca ai",which_scope_type="SCA")
    ...
    def test_assign_scans_and_check_ai(self, driver, engagement_name_xpath="//label[contains(text(),'check sca ai')]"):
        """
