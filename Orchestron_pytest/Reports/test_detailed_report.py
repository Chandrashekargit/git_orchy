...
@mark.check_detailed_report_filters
class reports_severity_count_Tests:
    ...
    def test_affected_instance_severity_with_severity_filter_option_selected(self, driver, app_name="//label[contains(text(), 'check detailed ..')]/../..//span[@class='el-checkbox__inner']"):
        ...
        for (a, b) in zip([high_sev, med_sev, low_sev, info_sev], [1, 2, 3, 4]):
            ...
            try:
                stop_till_spinner_is_invisible(driver)
                move_to_last_page = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.XPATH, "//button[@aria-label='Go to last page']")))
                ...
                for j in range(1, int(get_the_max_len_of_pagination) + 1):
                    ...
                    time.sleep(2)
                    severity_selected = wait.until(EC.presence_of_element_located((By.XPATH,
                                       "//div[@role='group']/div[@class='row']//div[@class='col-sm-6']["+str(b)+"]/label//span[2]/label")))
                    get_the_count_of_number_of_vul = wait.until(EC.presence_of_all_elements_located((By.XPATH, "//div[@class='list-group']/button")))

                    for k in range(1, len(get_the_count_of_number_of_vul) + 1):
                        severity_of_vul = wait.until(EC.presence_of_element_located((By.XPATH,
                                      "//div[@class='list-group']//button["+str(k)+"]"
                ...
                severity_selected = wait.until(EC.presence_of_element_located((By.XPATH,
                                       "//div[@role='group']/div[@class='row']//div[@class='col-sm-6']["+str(b)+"]/label//span[2]/label")))
                try:
                    get_the_count_of_number_of_vul = WebDriverWait(driver, 5).until(EC.presence_of_all_elements_located((By.XPATH, "//div[@class='list-group']/button")))
                    for l in range(1, len(get_the_count_of_number_of_vul) + 1):
                        stop_till_spinner_is_invisible(driver)
                        severity_of_vul = wait.until(EC.presence_of_element_located((By.XPATH,
                                          "//div[@class='list-group']//button["+str(l)+"]"
                                          "//span[@class='badge high move_right orchy_font_family  orchy_font_sm badge-secondary' "
                                          "or @class='badge medium move_right orchy_font_family  orchy_font_sm badge-secondary' "
                                          "or @class='badge low move_right orchy_font_family  orchy_font_sm badge-secondary' "
                                          "or @class='badge info move_right orchy_font_family  orchy_font_sm badge-secondary']")))
                        ...
                        click_on_individual_vul = wait.until(EC.element_to_be_clickable((By.XPATH, "//div[@class='list-group']/button["+str(l)+"]")))
                        click_on_individual_vul.click()
